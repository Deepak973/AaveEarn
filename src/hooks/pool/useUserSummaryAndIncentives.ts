...
export type FormattedUserReserves =
  ComputedUserReserve<FormattedReservesAndIncentives>;
...
export const useUserSummariesAndIncentives = (
  ...
  const userPoolsReservesQuery = useUserPoolsReservesHumanized(
    marketsData,
    user
  );
  const formattedReserves = usePoolsFormattedReserves(marketsData);
  const poolsReservesIncentivesQuery =
    usePoolsReservesIncentivesHumanized(marketsData);
  const userPoolsReservesIncentiveQuery =
    useUserPoolsReservesIncentivesHumanized(marketsData, user);
...
export const useUserSummaryAndIncentives = (
  ...
  return useUserSummariesAndIncentives([marketData], user)[0];
};
