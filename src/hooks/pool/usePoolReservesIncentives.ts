...
export const usePoolsReservesIncentivesHumanized = <
  T = ReservesIncentiveDataHumanized[]
>(
  ...
  return useQueries({
    queries: marketsData.map(
      (marketData) =>
        ({
          queryKey:
            queryKeysFactory.poolReservesIncentiveDataHumanized(marketData),
          queryFn: () =>
            uiIncentivesService.getReservesIncentivesDataHumanized(marketData),
