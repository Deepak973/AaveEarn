...
export const useExtendedUserSummariesAndIncentives = (
  ...
  const userSummariesQueries = useUserSummariesAndIncentives(marketsData, user);
  const userYieldsQueries = useUserYields(marketsData, user);
  const userReservesQueries = useUserPoolsReservesHumanized(marketsData, user);
...
export const useExtendedUserSummaryAndIncentives = (
  marketData: MarketDataType,
  user: `0x${string}`
) => {
  return useExtendedUserSummariesAndIncentives([marketData], user)[0];
};
