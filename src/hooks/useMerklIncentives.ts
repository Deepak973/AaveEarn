...
import { additionalIncentiveInfo } from "~/utils/addtional-incentive-infos";
...
const MERKL_ENDPOINT =
  "https://api.merkl.xyz/v4/opportunities?mainProtocolId=aave"; // Merkl API
...
export const useMerklIncentives = ({
  ...
  return useQuery({
    ...
    select: (merklOpportunities) => {
      const hardcodedIncentive = rewardedAsset
        ? hardcodedIncentives[rewardedAsset]
        : undefined;
      ...
      const opportunities = merklOpportunities.filter(
        (opportunitiy) =>
          ...
          opportunitiy.explorerAddress.toLowerCase() ===
            rewardedAsset.toLowerCase() &&
          protocolAction &&
